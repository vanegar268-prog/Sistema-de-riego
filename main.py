...
# 1) Manejar errores esperados de FastAPI 
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    return JSONResponse(
        ...
        content={
            ...
            "descripcion": "El servidor no puede procesar la petición debido a errores de validación.",
            "detalles": exc.errors(), #Lista detallada de errores
        },
    )

# 3) Errores no controlados 
...
async def global_exception_handler(request: Request, exc: Exception):
    return JSONResponse(
        ...
        content={
            ...
            "descripcion": (
                "La solicitud era válida, pero el servidor falló al procesarla. "
                "Esto indica un problema interno."
            ),
            "detalle": str(exc), #Descripcion tecmica del error
        },
    )
...
def actualizar_parcial(usuario_id: str, datos: UsuarioUpdate):
    """
    Actualiza parcialmente un usuario. Solo modifica los campos enviados.
    """
    usuario = get_usuario(usuario_id)
    datos_dict = datos.dict(exclude_unset=True)  # Solo los campos enviados
